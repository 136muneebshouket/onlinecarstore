...
const Order_service_form = ({heading}) => {
  ...
  let order_type = (url.split("/")[url.split("/").length - 1]).split('?')[0];

  let api_url = `/api/admin/inspection/inspec_orders/add_orders`
  // console.log(order_type)
  if(order_type == 'sell-it-for-me'){
    api_url = `/api/admin/inspection/inspec_orders/add_orders`
  }
    ...
    <>
      <form onSubmit={order_inspection}>
        ...
        <input
          ...
          value={
            inspection_obj?.city
              ? `${inspection_obj?.area} ${inspection_obj?.city}`
              : ""
          }
        ...
        />
        <span className="err_span">
          {showerr && `${inspection_obj?.Address ? "" : "Address is required"}`}
        </span>
        ...
        <input
          ...
          value={
            inspection_obj?.brand
              ? `${inspection_obj?.modelyear} ${inspection_obj?.brand} ${inspection_obj?.model} ${inspection_obj?.variant_name}`
              : ""
          }
...
export default memo(Order_service_form);
