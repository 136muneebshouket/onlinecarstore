...
const Upload_ad = () => {
      ...
      await axios
        ...
        .then(async (res) => {
          if (res.status == 201) {
            ...
            setImagestoshow([]);
            progress = 0;
            router.push(`/admin/dashboard_page/Inspection_sec/Start_inspec/Check_ad?Ad_id=${Ad_id}&managed_ad=true`)
          }
        })
        .catch((err) => {
          setMessage({ progress: null });
  ...
  const uploadcar = async (e) => {
    ...
    if (err_values.length == 0 && errors == false && phoneerr == false) {
      ...
      // console.log(cardata)
      await axios
        .post(`/api/admin/managed_cars/uplod_car`, cardata)
        .then(async (res) => {
          if (res.data.Ad_id) {
  ...
  return (
    <>
      ...
      <div className="upload_ad_page Confirm_ad">
